...
\renewcommand{\T}{{ \mathrm{\scriptscriptstyle T} }}
...
print_figs <-  1                            ## Print figures to file (leave =0)
...
### rho_\epsilon: init sample at a = 0.1, a = 2*pnorm(gamma) - 1
...
We can now start the Gibbs sampler, for which we will be requiring the conditional distributions. Denote $\theta$ as the collection of unknown parameters and denote $\rho_\varepsilon = 2\Phi(\tilde\rho_\varepsilon) - 1$, $r = \exp(\tilde{r})$ and $\kappa_i = \exp(\tilde\kappa_i)$, where the sign $~\~$ is used to note the (transformed) variable we actually sample from. Then
...
```{r,message=FALSE}
### DOMAIN PLOT
States <- maptools::readShapeSpatial("../../States/cb_2013_us_state_5m.shp")

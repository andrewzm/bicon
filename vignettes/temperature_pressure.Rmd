...
\textrm{Model 4 (diffused/shifted dependence):} &b_o(\h) \equiv \left\{\begin{array}{ll} A\{1 - (\|\h - \Deltab\|/r)^2\}^2, & \| \h - \Deltab\| \le r \\ 0, & \textrm{otherwise}, \end{array} \right. 
\end{array}
\end{equation*}
...
The first thing we do is set some options. is to indicate whether we want to run the full program. We use a flag `LOO_analysis` to indicate whether we want to run the full leave-one-out cross-validation study (that requires about 80 hours of computing time with no parallelisation) while the flag `LK_analysis` indicates whether we want to carry out a standard likelihood fit of all8 models using the entire dataset (requires about 30 minutes of computing time). In this vignette we set both flags to zero; the results are instead loaded.

```{r}
### Model choice
model_names <- c("independent","pointwise","moving_average_delta0","moving_average")
img_path <- "../paper/art"                  ## Where to save the figures
show_figs <- 0                              ## Show the figures in document
...
if(LOO_analysis) {
  ...
  pred <- foreach(i = 1:m1,.combine = "rbind",.packages = c("Matrix","bicon","dplyr","foreach")) %dopar% {
    
    #fit.Model <- fit_all_models(c(i,i+m1))
...
results2 <- pred2 %>%
  mutate(MAE = abs(mu_pred - Z),
        Bias = mu_pred - Z,
        SR = (mu_pred - Z)/sqrt(var_pred),
        CRPS = crps_wrapper(Z,mu_pred,sqrt(var_pred)))
```

Now we do LOO analysis with Gneiting's metric approach
```{r}
Dist.mat <- as.vector(RFearth2dist(as.matrix(weather[, 3:4])))
PT <- as.matrix(weather[, 1:2])
if(RF_estimation) {
  nug <- RMmatrix(M = matrix(nc = 2, c(NA, 0, 0, NA)), RMnugget())
  pars.model <- nug + RMbiwm(nudiag = c(NA, NA), scale = NA,
                                cdiag = c(NA, NA), rhored = NA)
  RFpars <- RFfit(pars.model, distances = Dist.mat, dim = 3, data = PT)
  print(RFpars)
  CVresults.pars <- RFcrossvalidate(RFpars, 
...
if(show_figs) {
  ...
  print(LinePlotTheme() + stat_qq(data=filter(pred2,model_num %in% c(1,4)),aes(sample=(mu_pred - Z)/sqrt(var_pred),shape=process,colour=as.factor(model_num)),size=4) + geom_abline())
  
  print(LinePlotTheme() + geom_bar(data=best_performing,aes((best_model_MAE - 1)%%4 + 1,fill=c(process)),position="dodge",binwidth=0.5) + 
    scale_fill_grey(guide_legend(title="process")) +
    xlab("best model for MAE"))
  
  
  print(LinePlotTheme() + geom_bar(data=best_performing,aes((best_model_CRPS - 1)%%4 + 1,fill=c(process)),position="dodge",binwidth=0.5) + 
    scale_fill_grey(guide_legend(title="process")) + 
...
g1 <- LinePlotTheme() + geom_tile(data=XY,aes(h1,h2,fill=b1)) + greys + axes + coord_fixed() +ggtitle("Model 4")
g2 <- LinePlotTheme() + geom_tile(data=XY,aes(h1,h2,fill=b2)) + axes + greys + coord_fixed() +ggtitle("Model 8")
...
Disp$corr <- foreach(i = 1:nrow(Disp),.combine="c") %dopar% {
  Temp <- akima::interp(mesh_locs[,1]-Disp$h1[i],mesh_locs[,2]-Disp$h2[i],Mesh["y1_Model1"],xo,yo)
  Pres <- akima::interp(mesh_locs[,1],mesh_locs[,2],Mesh["y2_Model1"],xo,yo)
...
std_diff <- (plot_interp(Mesh,"diffT_errors",150) + 
               scale_fill_gradient2(low=muted("yellow"),mid="white", high=muted("magenta"),guide =
                                      guide_legend(title="diff (deg. C)"))) %>% 
...
std_diff_scatter <- LinePlotTheme() + 
  geom_point(data=X,aes(M1_errors,M4_errors,fill = M1_errors - M4_errors),colour="black",size=4,shape=21) +    scale_fill_gradient2(low=muted("green"),mid="white", high=muted("magenta"),
                     guide=guide_legend(title="diff (deg. C)")) +
#   geom_point(data=X,aes(M1_errors,M4_errors,fill = M1_errors/M4_errors),colour="black",size=4,shape=21) +    scale_fill_gradient(low="green",high="red",limits=c(0.77,1.5),
#                      guide=guide_legend(title="diff (deg. C)")) +
  xlab("Model 1 standard errors for Y1 (deg. C)") +
  ylab("Model 4 standard errors for Y1 (deg. C)") +
  theme(text = element_text(size = 30), axis.title.y = element_text(vjust=2));std_diff_scatter



if(print_figs) {
  g <- arrangeGrob(g1M4,(std_diff_scatter + theme(plot.margin = grid::unit(c(10, 10, 10, 10),units="mm"))),g2M4,std_diff,ncol=2) 
  ggsave(g, 

...
\begin{itemize}
\item \textcolor{red}{Directed acyclic graphs} (DAGs) on the variables define a partial order that generalises the $p$-variate model. The conditional covariances, $\{C_{q|(r<q)}\}$, are replaced by a more parsimonious set $\{C_{q|pa(q)}\}$, where $pa(q)$ denotes the ``parents'' of the $q$-th variable.
...
\end{center}
\end{figure}
\textcolor{red}{Bivariate system}: We need to specify \textcolor{red}{three} marginal/cross-covariance functions. Recall that $C_{12}(\s,\u)=C_{21}(\u,\s)$, which is not the symmetry constraint.

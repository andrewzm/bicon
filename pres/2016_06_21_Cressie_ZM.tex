...
%%% Using Natbib: (open, close, between multiple citations, author-year style, (author symbol year), (year symbol year multiple citations)
\bibpunct{(}{)}{;}{a}{,}{,}
%%%
...
% \subtitle{``An application to flux estimation"} %Not required if you don't want it
 \author[N. Cressie \& A. Zammit-Mangion]{\textbf{Noel Cressie} 
 \\ and 
 \\ Andrew Zammit-Mangion \vfill
 National Institute for Applied Statistics Research Australia 
 \\  University of Wollongong  \vfill
 ncressie@uow.edu.au} %
  \institute[]{\vspace{0.4in} \includegraphics[height=1.2cm]{NiasraUowLonghand}} % Substitute date of presentation or include todays date instead.
\date{}
\begin{document}
...
\begin{itemize}
    ...
    \item Determine which variable caused the observed phenomenon: \textcolor{red}{Source separation} (not considered in this talk)\vfill 
\end{itemize}\vfill
\end{itemize}
...
\begin{itemize}
\item \textcolor{red}{Directed acyclic graphs (DAGs)} on the variables define a partial order that allows a more parsimonious $p$-variate model. The conditional covariances, $\{C_{q|(r<q)}\}$, are replaced by the parsimonious set $\{C_{q|pa(q)}\}$, where $pa(q)$ denotes the ``parents'' of the $q$-th variable.
\item Computationally efficient algorithms are available for DAGs.
\end{itemize}
\red{Examples of DAGs} include:
\begin{figure}[!t]
\begin{center}
...
\end{center}
\end{figure}
\textcolor{red}{Bivariate system}: We need to \textcolor{red}{specify three} marginal/cross-covariance functions that result in a \textcolor{red}{nnd CCFM}. Recall that $C_{12}(\s,\u)=C_{21}(\u,\s)$, which is not the symmetry constraint.

Building blocks for the conditional approach: \textcolor{red}{Three} functions, nnd $C_{11}(\cdot,\cdot)$, nnd $C_{2\mid 1}(\cdot,\cdot)$, integrable $b(\cdot,\cdot)$, \textcolor{red}{specified independently}.
...
\end{center}
\end{figure}
\textcolor{red}{Trivariate system}: Need to \textcolor{red}{specify six} marginal/cross-covariance functions that result in a \textcolor{red}{nnd CCFM}. Recall that $C_{ij}(\s,\u)=C_{ji}(\u,\s)$.

Building blocks for the conditional approach: \textcolor{red}{Six} functions, $C_{11}(\cdot,\cdot)$, $C_{2\mid 1}(\cdot,\cdot)$, $C_{3|1,2}(\cdot,\cdot)$, $b_{21}(\cdot,\cdot)$, $b_{31}(\cdot,\cdot)$, $b_{32}(\cdot,\cdot)$, \textcolor{red}{specified independently}.
